...
class Color666MPSDecoderXXXZZZSimulated(Decoder):
    """
    Implements a planar Matrix Product State (MPS) decoder for the effective XXXZZZ color code.
    """
    ...
    def decode(self, code, syndrome,
        ...
        # prob_dist = error_model.probability_distribution_test(error_probability)
    ...
    class TNC:
        ...
        @functools.lru_cache()  # 256=2^8 is enough for fixed prob_dist
        def create_q_node(self, ps, fs, flips, compass_direction=None):
            """
            Return qubit tensor.
            ...
            for shape, f, flip in zip(_node_shapes(compass_direction), fs, flips):
                ...
                if shape is None:  # add dummy tensor
                    ...
                    #    node[(n, e, s, w)] = self.q_node_value(p, f, n, e, s, w)
                    #nodes.append(node)

                    test = ps[0]
                    if flip == False:
                        for n, e, s, w in np.ndindex(node.shape):
                            node[(n, e, s, w)] = self.q_node_value(ps[1], f, n, e, s, w)
                    else:
                        for n, e, s, w in np.ndindex(node.shape):
                            node[(n, e, s, w)] = self.q_node_value(ps[0], f, n, e, s, w)
                    nodes.append(node)
        ...
        def create_tn(self, prob_dist, sample_pauli):
            ...
            prob_dist_swap = (prob_dist[0], prob_dist[3], prob_dist[2], prob_dist[1])
            ps = (prob_dist, prob_dist_swap)
            # initialise empty tn
            ...
            for c in range(code.bound + 1):  # iterate pauli columns
                ...
                while r <= code.bound:  # iterate pauli rows
                    if code.is_site((r, c)):  # create node for site(s)
                        ...
                        if c == 0:  # left side
                            if r == 0:
                                node = self.create_q_node(ps, (f1, f2), (F, T), 'nw')
                            elif r == code.bound:
                                node = self.create_q_node(ps, (f1, None), (F, T), 'sw')
                            else:
                                node = self.create_q_node(ps, (f1, f2), (F, T), 'w')
                        elif r == c:  # diagonal side
                            if c == code.bound:
                                node = self.create_q_node(ps, (f1, None), (F, T), 'e')
                            elif c % 3 == 2:
                                node = self.create_q_node(ps, (None, f1), (F, T), 'ne')
                            else:
                                node = self.create_q_node(ps, (f1, f2), (F, T), 'n')
                        elif r == code.bound - 1:  # lower side - 1 because we merge in lower qubits
                            node = self.create_q_node(ps, (f1, f2), (F, T), 's')
                        elif r == code.bound:  # lower side
                            node = self.create_q_node(ps, (f1, None), (F, T), 'se')
                        else:  # bulk
                            node = self.create_q_node(ps, (f1, f2), (F, T))
                        if f2 is not None:
