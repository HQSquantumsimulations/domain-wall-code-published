...
class Color666MPSDecoder(Decoder):
    """
    Implements a planar Matrix Product State (MPS) decoder for the X3Z3 code.
    *for 3 out of 6 qubits in each stabilizer, Xs and Zs are interchanged as compared to the CSS code
                ...
                if step:
                    for cc in range(c1, c2 + step, step):
                        if code.is_site((r1, cc)):
                            if op == 'X':
                                if (cc + r1) % 6 == 3 or (cc + r1) % 6 == 4:
                                    if r1 % 2 == 0:
                                        operation = 'X'
                                    else:
                                        operation = 'Z'
                                elif (cc + r1) % 6 == 0 or (cc + r1) % 6 == 1:
                                    if r1 % 2 == 0:
                                        operation = 'Z'
                                    else:
                                        operation = 'X'
                                ...
                                if (cc + r1) % 6 == 3 or (cc + r1) % 6 == 4:
                                    ...
                                    if r1 % 2 == 0:
                                        ...
                                        operation = 'Z'
                            sample_recovery.site(operation, (r1, cc))
                # path along vertical
                step = np.sign(r2 - r1)
                if step:
                    for rr in range(r1, r2 + step, step):
                        if code.is_site((rr, c1)):
                            if op == 'X':
                                if (c1 + rr) % 6 == 3 or (c1 + rr) % 6 == 4:
                                    if rr % 2 == 0:
                                        operation = 'X'
                                    else:
                                        operation = 'Z'
                                elif (c1 + rr) % 6 == 0 or (c1 + rr) % 6 == 1:
                                    if rr % 2 == 0:
                                        operation = 'Z'
                                    else:
                                        operation = 'X'
                                ...
                                if (c1 + rr) % 6 == 3 or (c1 + rr) % 6 == 4:
                                    ...
                                    if rr % 2 == 0:
                                        ...
                                        operation = 'Z'
                            sample_recovery.site(operation, (rr, c1))
        # return sample
    ...
    def decode(self, code, syndrome,
        ...
        return max_recovery.to_bsf()# , test_recovery.to_bsf()
    ...
    class TNC:
        ...
        @functools.lru_cache(maxsize=2 ** 28)
        def q_node_value_flip(self, prob_dist, f, n, e, s, w):
            """Return qubit tensor element value."""
            paulis = ('I', 'X', 'Y', 'Z')
            op_to_pr = dict(zip(paulis, prob_dist))
            f = pt.pauli_to_bsf(f)
            # n, e, s, w are in {0, 1, 2, 3} so create dict from index to op
            paulis_2 = ('I', 'Z', 'Y', 'X')
            index_to_op = dict(zip((0, 1, 2, 3), pt.pauli_to_bsf(paulis_2)))
            # apply ops from indices to f
            op = (f + index_to_op[n] + index_to_op[e] + index_to_op[s] + index_to_op[w]) % 2
            # return probability of op
            return op_to_pr[pt.bsf_to_pauli(op)]

        @functools.lru_cache(maxsize=2 ** 28)  # 256=2^8 is enough for fixed prob_dist
        def create_q_node(self, prob_dist, fs, flips, compass_direction=None):
            """
            Return qubit tensor.
            ...
            nodes = []
            for shape, f, flip in zip(_node_shapes(compass_direction), fs, flips):
                ...
                if shape is None:  # add dummy tensor
                    ...
                    if flip == False:
                        ...
                        for n, e, s, w in np.ndindex(node.shape):
                            node[(n, e, s, w)] = self.q_node_value_flip(prob_dist, f, n, e, s, w)
                    nodes.append(node)
        ...
        def create_tn(self, prob_dist, sample_pauli):
            ...
            for c in range(code.bound + 1):  # iterate pauli columns
                ...
                F and T apply the correct permutations between Xs and Zs when decoding
                """
                if c % 2 == 0:
                    F = False
                    T = True
                ...
                while r <= code.bound:  # iterate pauli rows
                    if code.is_site((r, c)):  # create node for site(s)
                        ...
                        if c == 0:  # left side
                            if r == 0:
                                node = self.create_q_node(prob_dist, (f1, f2), (F, T), 'nw')
                            elif r == code.bound:
                                node = self.create_q_node(prob_dist, (f1, None), (F, T), 'sw')
                            else:
                                node = self.create_q_node(prob_dist, (f1, f2), (F, T), 'w')
                        elif r == c:  # diagonal side
                            if c == code.bound:
                                node = self.create_q_node(prob_dist, (f1, None), (F, T), 'e')
                            elif c % 3 == 2:
                                node = self.create_q_node(prob_dist, (None, f1), (F, T), 'ne')
                            else:
                                node = self.create_q_node(prob_dist, (f1, f2), (F, T), 'n')
                        elif r == code.bound - 1:  # lower side - 1 because we merge in lower qubits
                            node = self.create_q_node(prob_dist, (f1, f2), (F, T), 's')
                        elif r == code.bound:  # lower side
                            node = self.create_q_node(prob_dist, (f1, None), (F, T), 'se')
                        else:  # bulk
                            node = self.create_q_node(prob_dist, (f1, f2), (F, T))
                        if f2 is not None:
