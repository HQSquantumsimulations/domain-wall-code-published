...
class Color666MPSDecoderXXZXXZSimulated(Decoder):
    """
    Implements a planar Matrix Product State (MPS) decoder for the effective XXZXXZ color code.
    """
    ...
    class TNC:
        ...
        def create_q_node(self, ps, fs, flip, compass_direction=None):
            """
            Return qubit tensor.
            ...
            for c in range(code.bound + 1):  # iterate pauli columns
                ...
                while r <= code.bound:  # iterate pauli rows
                    ...
                    if code.is_site((r, c)):  # create node for site(s)
                        ...
                        if c == 0:  # left side
                            if r == 0:
                                node = self.create_q_node(ps, (f1, f2), F, 'nw')
                            elif r == code.bound:
                                node = self.create_q_node(ps, (f1, None), F, 'sw')
                            else:
                                node = self.create_q_node(ps, (f1, f2), F, 'w')
                        elif r == c:  # diagonal side
                            if c == code.bound:
                                node = self.create_q_node(ps, (f1, None), F, 'e')
                            elif c % 3 == 2:
                                node = self.create_q_node(ps, (None, f1), F, 'ne')
                            else:
                                node = self.create_q_node(ps, (f1, f2), F, 'n')
                        elif r == code.bound - 1:  # lower side - 1 because we merge in lower qubits
                            node = self.create_q_node(ps, (f1, f2), F, 's')
                        elif r == code.bound:  # lower side
                            node = self.create_q_node(ps, (f1, None), F, 'se')
                        else:  # bulk
                            node = self.create_q_node(ps, (f1, f2), F)
                        if f2 is not None:
