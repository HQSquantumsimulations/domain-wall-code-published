...
class PhaseFlipErrorModel(SimpleErrorModel):
    """
    EFFECTIVE ERROR MODEL FOR SIMULATING THE XXZXXZ CODE
    * The XXZXXZ code is simulated with the CSS code and effective error model
...
class BiasedDepolarizingErrorModel(PhaseFlipErrorModel):
    ...
    * The XXZXXZ code is simulated with the CSS code and effective error model
    ...
    def label(self):
        """See :meth:`qecsim.model.ErrorModel.label`"""
        return 'Biased-depolarizing, efficient model for XXZXXZ (bias={!r}, axis={!r})'.format(self._bias, self._axis)
