...
class PhaseFlipErrorModel(SimpleErrorModel):
    """
    EFFECTIVE ERROR MODEL FOR SIMULATING THE XXXXZZ CODE
    * The XXXXZZ code is simulated with the CSS code and effective error model
    by interchanging X and Z errors within domains.
    """
    ...
    def label(self):
        ...
        return 'Phase-flip errors, efficient model for XXXXZZ'
...
class BiasedDepolarizingErrorModel(PhaseFlipErrorModel):
    ...
    * The XXXXZZ code is simulated with the CSS code and effective error model
    ...
    def label(self):
        """See :meth:`qecsim.model.ErrorModel.label`"""
        return 'Biased-depolarizing, efficient model for XXXXZZ (bias={!r}, axis={!r})'.format(self._bias, self._axis)
